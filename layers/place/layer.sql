...
CREATE OR REPLACE FUNCTION layer_place (
    bbox geometry,
    zoom_level int,
    pixel_width numeric
)
    ...
    FROM (
        ...
        WHERE
            ...
            AND (zoom_level >= 5
                OR is_in_country IN ('United Kingdom', 'USA', 'Россия', 'Brasil', 'China', 'India')
        ...
        WHERE
            geometry && bbox
            AND ((zoom_level = 8
                    AND island_rank (area) <= 3)
                OR (zoom_level = 9
                    AND island_rank (area) <= 4)
                OR (zoom_level >= 10))
        UNION ALL
        SELECT
            ...
            NULL::text AS iso_a2
        FROM
            layer_city (bbox, zoom_level, pixel_width)
        ORDER BY
            "rank" ASC) AS place_all
$$
LANGUAGE SQL
