...
CREATE OR REPLACE VIEW landuse_z4 AS (
    SELECT NULL::bigint AS osm_id, geometry, NULL::text as name, 'residential'::text AS landuse, NULL::text AS amenity, NULL::text AS leisure, NULL::text AS tourism, NULL::text AS place, NULL::text AS waterway
...
CREATE OR REPLACE VIEW landuse_z6 AS (
    SELECT osm_id, geometry, name, landuse, amenity, leisure, tourism, place, waterway
...
RETURNS TABLE(osm_id bigint, geometry geometry, name text, class text) AS $$
    SELECT osm_id, geometry,
        case when (NULLIF(landuse, '') is not null and landuse not in ('residential', 'cemetery')) then nullif(name,'') end as name,
       COALESCE(
               NULLIF(landuse, ''),
               NULLIF(amenity, ''),
               NULLIF(leisure, ''),
